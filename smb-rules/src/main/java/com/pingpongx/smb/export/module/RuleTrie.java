...
public class RuleTrie extends Trie<RuleLeaf, RuleTrieElement>{
    ...
    public MatchResult match(Object data){
        ...
        while(!bfsQ.isEmpty()){
            ...
            matchedRule.stream()
                    .flatMap(node -> node.getData().getChildOperations().stream())
                    .filter(operation -> !matchedOp.contains(operation)).forEach(operation -> {
                        BatchMatcher toAdd = engine.batchMatcherMapper.routeMatchers(operation);
                        bfsQ.put(toAdd,operation);
                    });
        }
        Set<RuleHandler>  handlerSet =  matchedRulesRepeat.stream().map(matched->matched.getData())
    ...
    private RuleTrie putAnd(RuleAnd and,RuleHandler handler){
        ...
        for (int i=0;i<ids.size();i++){
            ...
            if (matcher==null){
                matcher = BatchMatcherFactory.newBatchMatcher((String) op.getIdentify());
