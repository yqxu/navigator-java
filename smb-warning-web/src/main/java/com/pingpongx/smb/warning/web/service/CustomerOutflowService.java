...
public class CustomerOutflowService {
    ...
    public void regularWarn() {
        ...
        List<JiraInfo> P0jiraInfos = jiraInfoDao.query(jira2vClientConfig.getProject(), "已解决", JiraInfo.PriorityLevel.P0.name(), LocalDateUtils.time(zonedDateTime));
        zonedDateTime = zonedDateTime.minusDays(7);
        List<JiraInfo> P1jiraInfos = jiraInfoDao.query(jira2vClientConfig.getProject(), "已解决", JiraInfo.PriorityLevel.P1.name(), LocalDateUtils.time(zonedDateTime));
